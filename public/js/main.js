window.onload = function(){
  ...
  newUpdateSearchUrl.addEventListener('mouseover', function(e){
    ...
    switch(el.id){
      case 'passwordUrl': 
        ...
        'To update URL in the future' : 'Enter password set during creation';break;
      case 'longUrl': 
        el.placeholder = newUpdateSearchUrl.classList.contains('new')? 'Example: https://www.google.com': 
        newUpdateSearchUrl.classList.contains('update')?'Optional: (Example) https://www.google.com' : 'Examples: gcloud,a,new...';
        break;
      case 'shortUrl': el.placeholder = 's.dsce.in/<desired short url>';break;
      case 'passwordUrlUpdate': el.placeholder = 'Optional: To update URL in the future';break;
      case 'shortUrlUpdate': el.placeholder = 'Optional: s.dsce.in/<desired short url>';break;
    }
  });
  newUpdateSearchUrl.addEventListener('mouseout', function(e){
    ...
    switch(el.id){
      case 'passwordUrl': 
        el.placeholder = newUpdateSearchUrl.classList.contains('new')? 
        'Enter Password' : 'Enter Old Password';break;
      case 'longUrl': 
        el.placeholder = newUpdateSearchUrl.classList.contains('new')? 'Enter Long URL': 
        newUpdateSearchUrl.classList.contains('update')? 'Enter New Long URL' : 'Key to Search';
        break;
      case 'shortUrl': el.placeholder = 'Enter Desired Short URL';break;
      case 'passwordUrlUpdate': el.placeholder = 'Enter New Password';break;
      case 'shortUrlUpdate': el.placeholder = 'Enter New Desired Short URL';break;
    }
  });
  ...
  function submitNew(){
    ...
    if(entered_shortUrl.length != 0 && (entered_shortUrl.length < MIN_LENGTH_SHORTURL || entered_shortUrl.length > MAX_LENGTH_SHORTURL)) {
      newOrUpdateMessage(true, `The desired short url should be ${MIN_LENGTH_SHORTURL}-${MAX_LENGTH_SHORTURL} characters long`, "new");
      return;
    } else {
    ...
    axios.post(`${protocol+"//"+domain}/api/update/new`, obj)
    .then(res=>{
      if(res.data){
        if(res.data.error){
  ...
  function submitUpdate(){
    ...
    axios.post(`${protocol+"//"+domain}/api/update/modify`, obj)
    .then(res=>{
      if(res.data){
        if(res.data.error){
  ...
  function submitSearch(){
    ...
    axios.get(`${protocol+"//"+domain}/api/search/${encodeURIComponent(searchString)}`)
    .then(res=>{
      if(res.data){
        if(res.data.error){
          ...
          let html = exact.map((url) => {
            let longUrl = url.longUrl.split(new URL(url.longUrl).protocol+"//")[1];
            let row =  
            `<tr ${url.exact?`style="background-color: #F1F1F1"`:``}>`+
              `<td><a href="${protocol+"//"+domain}/${url.shortUrl}">s.dsce.in/${url.shortUrl}</a></td>`+
              `<td><a href="${url.longUrl}">${longUrl.length > 64 ? longUrl.substring(0,61)+"...":longUrl}</a></td>`+
